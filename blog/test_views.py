...
class TestBlogViews(TestCase):
    ...
    def test_add_to_read_later(self):
        ...
        response = self.client.post(
            reverse(
                'add_to_read_later', args=[
                    self.post.slug]))

        # Assert that the response is a redirect (302)
        self.assertEqual(response.status_code, 302)

        # Follow the redirect to the target page (typically the Read Later list
        # page)
    ...
    def test_remove_from_read_later(self):
        ...
        # Now, make the POST request to remove the post from the Read Later
        # list
        response = self.client.post(
            reverse(
    ...
    def test_upvote_post(self):
        ...
        # Upvote the post
        response = self.client.post(
            reverse('post_upvote', args=['blog-title']))
